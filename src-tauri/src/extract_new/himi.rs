...
impl HIMINewExtractor {
    ...
    fn build_category_buf_filename_map_for_index(
        &self,
        ...
        for category_name in &d3d11_game_type.ordered_category_name_list {
            let topology = d3d11_game_type
                ...
                .cloned()
                .unwrap_or_default();
            let mut extract_index = trianglelist_index.to_string();
        ...
        Ok(Some(category_buf_filename_map))
    }

    fn find_export_ib_for_game_type(
        &self,
        ...
        let target_ib_txt_file = IndexBufferTxtFile::new(&ib_txt_file_path, false)?;
        let target_first_index = target_ib_txt_file.first_index.parse::<u64>().unwrap_or(0);

        for trianglelist_index in trianglelist_index_list {
            let candidate_ib_txt_file_name = self
                .fa
                .data
                .filter_first_file(&format!("{}-ib", trianglelist_index), ".txt")
                .unwrap_or_default();
            if candidate_ib_txt_file_name.is_empty() {
                continue;
            }

            let candidate_ib_txt_file_path = self
                .fa
                .log
                .get_deduped_filepath(&candidate_ib_txt_file_name);
            if candidate_ib_txt_file_path.is_empty() {
                continue;
            }

            let candidate_ib_txt_file =
                IndexBufferTxtFile::new(&candidate_ib_txt_file_path, false)?;
            let candidate_first_index = candidate_ib_txt_file
                .first_index
                .parse::<u64>()
                .unwrap_or(0);
            if candidate_first_index != target_first_index {
                continue;
            }

            if let Some(category_buf_filename_map) = self
                .build_category_buf_filename_map_for_index(
                    trianglelist_index,
                    pointlist_index,
                    d3d11_game_type,
                )?
    ...
    fn export_unity_vs_submeshes(
        ...
        for d3d11_game_type in possible_d3d11_game_type_list {
            ...
            for ib_txt_file_name in match_first_index_ib_txt_file_name_dict.values() {
                let Some((ib_txt_file_name, category_buf_filename_map)) = self
                    .find_export_ib_for_game_type(
                        ib_txt_file_name,
                        trianglelist_index_list,
                        pointlist_index,
                        d3d11_game_type,
    ...
    fn new_internal(
        ...
        let current_gametype_folder_path = gametype_folder_path.join("HIMI");
    ...
    pub fn get_possible_gametype_list_unity_vs(
        ...
        if possible_gametype_list.is_empty() {
            ...
            for gt in possible_gametype_list.iter() {
                let category_count = gt.category_draw_category_dict.len();
                if max_category_count < category_count {
    ...
    pub fn run_extract(
        ...
        self.sync_himi_textures_and_json(draw_ib_list)?;
        Ok(())
    }
    ...
    ///  使用 `index_count` 而非 `index_number_count`），
    /// 确保生成的子网格文件夹名与模型提取一致。
    fn sync_himi_textures_and_json(&self, draw_ib_list: Vec<String>) -> Result<(), String> {
        ...
        for entry in drawib_config.entries.iter() {
            ...
            for (&first_index, ib_txt_file_name) in match_first_index_ib_map.iter() {
                ...
                let ib_txt_file = IndexBufferTxtFile::new(&ib_txt_file_path, true)?;
                // 使用 index_count（头文件值）以匹配模型提取的文件夹命名
                let submesh_index_count = ib_txt_file.index_count.clone();
