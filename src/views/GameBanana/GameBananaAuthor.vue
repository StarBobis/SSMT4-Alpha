...
<template>
  <div class="page-container gb-author-page">
    ...
    <section class="gb-author-toolbar glass-panel">
      ...
      <label class="gb-author-hide-field"><span>{{ t('gameBanana.nsfwShown') }}</span><div><el-switch :model-value="appSettings.gamebananaHideNsfw" @change="(value: string | number | boolean) => setGameBananaHideNsfw(appSettings, value === true)" /><span>{{ appSettings.gamebananaHideNsfw ? t('gameBanana.hideNsfw') : t('gameBanana.showNsfw') }}</span></div></label>
...
.gb-author-hide-field { display:grid; min-width:92px; gap:4px; }.gb-author-hide-field>span { color:rgba(var(--theme-text-secondary-rgb),.64); font-size:10px; font-weight:700; text-transform:uppercase; }.gb-author-hide-field>div { display:flex; align-items:center; gap:7px; min-height:30px; padding:0 8px; border:1px solid rgba(255,255,255,.13); border-radius:7px; background:rgba(255,255,255,.055); box-sizing:border-box; }.gb-author-hide-field>div>span { color:rgba(var(--theme-text-primary-rgb),.82); font-size:11px; white-space:nowrap; }.gb-author-nsfw-mode :deep(.el-radio-button.is-disabled .el-radio-button__inner) { opacity:.35; cursor:not-allowed; }
